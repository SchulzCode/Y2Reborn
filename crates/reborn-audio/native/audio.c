/* SPDX-License-Identifier: MIT — single-owner ALSA ABI wrapper. */
...
int rb_wired_device(char *out, int n) {
  ...
  while ((r = snd_card_next(&card)) >= 0 && card >= 0) {
    ...
    if (match)
      snprintf(out, n, "hw:CARD=Y2Audio,DEV=0");
    snd_ctl_close(ctl);
    if (match)
      return 0;
  }
...
int rb_sink_open(const char *name, unsigned rate, int wired, RbSink **out,
                 RbParams *params) {
  int r;
  *out = NULL;
  ...
  if (access("/run/y2", F_OK) == 0) {
    s->lease =
        open("/run/y2/activity.lock", O_CREAT | O_RDWR | O_CLOEXEC, 0600);
  ...
  if ((r = snd_pcm_hw_params_set_access(s->pcm, p,
    ...
    goto fail;
  if ((r = snd_pcm_hw_params_set_format(s->pcm, p, SND_PCM_FORMAT_S16_LE)) < 0)
    goto fail;
  ...
  if ((r = snd_pcm_hw_params_set_period_size_near(s->pcm, p, &period, &dir)) <
      0)
    goto fail;
...
int rb_sink_write(RbSink *s, const int16_t *p, int frames) {
  return (int)snd_pcm_writei(s->pcm, p, frames);
}
int rb_sink_wait(RbSink *s, int ms) { return snd_pcm_wait(s->pcm, ms); }
int rb_sink_recover(RbSink *s, int code) {
  return snd_pcm_recover(s->pcm, code, 1);
}
...
int rb_sink_start(RbSink *s) { return snd_pcm_start(s->pcm); }
